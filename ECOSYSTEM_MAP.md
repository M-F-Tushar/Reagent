...
This document describes the as-built architecture at the end of Week 16, once
all sixteen weeks are applied. It is not a plan for future work; every package,
route, and service named below exists in the repository today.
...
This tree matches `src/reagent` as it stands; there is no `services/` package.
Application-level orchestration lives directly in `api/` and in the
`reagent-week-NN` scripts under `scripts/`, rather than behind a separate
use-case layer.
